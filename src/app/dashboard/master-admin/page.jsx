...
export default function MasterAdminDashboard() {
  ...
  const getPlanBadge = (plan, approvalStatus) => {
    ...
    return (
      <div className="flex items-center gap-2">
        <Badge className={planColors[plan] || "bg-gray-500"}>
          {plan.toUpperCase()}
        </Badge>
  ...
  const activeUsers = users.filter(u => {
    // Count as active if:
    // 1. Explicitly approved and active
    // 2. Personal plan (auto-approved) and active
    // 3. No planApprovalStatus field (legacy users) and active
    const isApproved = u.planApprovalStatus === "approved" || 
                       u.membershipPlan === "personal" || 
                       !u.planApprovalStatus;
    return isApproved && u.isActive;
  });
  const blockedUsers = users.filter(u => !u.isActive && u.planApprovalStatus !== "pending");
      ...
      <SidebarInset>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Enterprise plans waiting</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-3">
            ...
            <p className="text-xs text-muted-foreground">Inactive accounts</p>
              ...
              <Tabs defaultValue="users" className="space-y-4">
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Manage user accounts, approve Enterprise plans, and control access
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <Table>
                          ...
                          {users.length === 0 ? (
                            ...
                            users.map((user) => (
                              <TableRow key={user._id}>
                                ...
                                <TableCell>{getPlanBadge(user.membershipPlan, user.planApprovalStatus)}</TableCell>
                                <TableCell>
                                ...
                                <TableCell>
                                    ...
                                    {user.role !== "master_admin" && user.planApprovalStatus !== "pending" && (
                                      <Button
                                        size="sm"
              ...
              <AlertDialog open={!!selectedAction} onOpenChange={() => setSelectedAction(null)}>
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>
                      {selectedAction?.type === "approve" && "Approve User Plan"}
                      {selectedAction?.type === "reject" && "Reject User Plan"}
                      {selectedAction?.type === "block" && "Block User"}
                      {selectedAction?.type === "unblock" && "Unblock User"}
                      {selectedAction?.type === "approve-vendor" && "Approve Vendor"}
                      {selectedAction?.type === "reject-vendor" && "Reject Vendor"}
                    </AlertDialogTitle>
                    <AlertDialogDescription>
