  ...
  try {
    ...
    if (session.user.role === "master_admin") {
      ...
      if (vendorProfile) {
        ...
        console.log(
          "Public vendor - fetching products for vendor ID:",
          vendorProfile._id,
        );
      } else {
      ...
      if (vendorId) {
        ...
        if (vendor && vendor.vendorType === "public") {
          ...
          const hasAddedVendor =
            vendor.addedByUsers && vendor.addedByUsers.includes(adminId);
          if (!hasAddedVendor) {
            ...
            console.log(
              "Regular user - fetching approved products from public vendor",
            );
          }
        } else {
          ...
          console.log(
            "Regular user - fetching products from private/virtual vendor",
          );
        }
      } else {
        ...
        const publicVendorIds = userVendors
          .filter((v) => v.vendorType === "public")
          .map((v) => v._id);
        const privateVendorIds = userVendors
          .filter((v) => v.vendorType !== "public")
          .map((v) => v._id);
        ...
        console.log(
          "Regular user - fetching products from all vendors with approval filtering",
        );
      }
    }
...
export async function POST(request) {
  try {
    ...
    if (!authorized)
      return NextResponse.json(
        { error: error || "Insufficient permissions to add products" },
        { status: 403 },
      );
    ...
    if (!sku || !name || !vendorId) {
      return NextResponse.json(
        { error: "Missing required fields: sku, name, vendorId" },
        { status: 400 },
      );
    }
